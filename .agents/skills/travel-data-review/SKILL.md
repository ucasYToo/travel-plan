...
description: |
  ...
  支持自动修正并生成结构化的审计报告保存到 .Codex/dataReview/。
tools:
  - Read
  - Write
  - Edit
...
- **报告必须生成**：每次执行后保存审计报告到 `.Codex/dataReview/YYYY-MM-DD-HH-MM-review-report.md`
...
7. **生成审计报告** → 写入 `.Codex/dataReview/` 并按需创建目录
...
| `.Codex/dataReview/*.md` | 审计报告输出目录 | Write |
...
```
/Users/niannian/seoul/.Codex/dataReview/YYYY-MM-DD-HH-MM-review-report.md
```
使用当前实际时间作为文件名（24小时制）。
